...
def analyze_text():
    output_text = ""
    text_input = input_path_entry.get()
    sentences = text_input.split('.')  # Split text into sentences
    emotions = []
    for sentence in sentences:
        if sentence.strip() != "":
            output = pipe_lr.predict([sentence.strip()])
            emotions.append((sentence.strip(), output[0]))
    output_text = "\n".join([f"Text: {sentence}. \n  Emotion: {emotion}" for sentence, emotion in emotions])
    output_label.config(text=output_text)
    root.update_idletasks()
...
custom_font = font.Font(family="Helvetica", size=100)
